...
// 	assignment := expect_and_unwrap(t, xar.get(&block.statements, 0), ^Write_Node)
// 	target := expect_and_unwrap(t, assignment.left, ^Variable_Read_Node)

// 	testing.expect_value(t, target.name, "x")
// }
...
// 	testing.expect_value(t, errs[1].type, Checker_Error_Type.Bad_Conversion)
// }
...
// @(test)
// test_error_on_forgotten_return :: proc(t: ^testing.T) {
// 	ast, err := parse_file("function foo(): int {  }", context.temp_allocator)
...
// 	testing.expect_value(t, len(errs), 1)
// 	testing.expect_value(t, errs[0].type, Checker_Error_Type.Bad_Control_Flow)
// }

// @(test)
// test_error_on_partial_forgotten_return :: proc(t: ^testing.T) {
// 	ast, err := parse_file("function foo(x: int): int { if x < 10 { return 5; } }", context.temp_allocator)
